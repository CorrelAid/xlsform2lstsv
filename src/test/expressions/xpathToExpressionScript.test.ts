...
describe('ExpressionConverter', () => {
	...
	describe('string functions', () => {
		test('converts string() function', () => {
			const result = converter.convert('string(${name}) = "John"');
			expect(result).toContain('name');
			expect(result).not.toContain('string(');
		});

		test('converts concat() function', () => {
			const result = converter.convert('concat(${first}, " ", ${last})');
		...
		test('converts substring() function', () => {
			const result = converter.convert('substring(${text}, 1, 5)');
			expect(result).toContain('substr(');
			expect(result).toContain('text');
		});

		test('converts string-length() function', () => {
			const result = converter.convert('string-length(${text}) > 0');
			expect(result).toContain('len(');
			expect(result).toContain('text');
		});

		test('converts contains() function', () => {
			const result = converter.convert('contains(${text}, "search")');
			expect(result).toContain('strpos(');
			expect(result).toContain('> 0');
		});

		test('converts starts-with() function', () => {
			const result = converter.convert('starts-with(${text}, "prefix")');
			expect(result).toContain('startsWith(');
		});

		test('converts ends-with() function', () => {
			const result = converter.convert('ends-with(${text}, "suffix")');
			expect(result).toContain('endsWith(');
		});
	});

	describe('number functions', () => {
		test('converts number() function', () => {
			const result = converter.convert('number(${age}) > 18');
			expect(result).toContain('age');
			expect(result).not.toContain('number(');
		});

		test('converts floor() function', () => {
			const result = converter.convert('floor(${value})');
			expect(result).toContain('floor(');
		});

		test('converts ceiling() function', () => {
			const result = converter.convert('ceiling(${value})');
			expect(result).toContain('ceil(');
		});

		test('converts round() function', () => {
			const result = converter.convert('round(${value})');
	...
	describe('math functions', () => {
		test('converts sum() function', () => {
			const result = converter.convert('sum(${values}) > 100');
			expect(result).toContain('sum(');
		});

		test('converts count() function', () => {
			const result = converter.convert('count(${items}) > 0');
	...
	describe('date functions', () => {
		test('converts today() function', () => {
			const result = converter.convert('today()');
			expect(result).toContain('today()');
		});

		test('converts now() function', () => {
			const result = converter.convert('now()');
	...
	describe('logical functions', () => {
		test('converts not() function', () => {
			const result = converter.convert('not(${field} = "")');
			expect(result).toContain('!(');
			expect(result).toContain('field');
		});
	});
	...
	describe('conditional functions', () => {
		test('converts if() function to ternary operator', () => {
			const result = converter.convert('if(${age} > 18, "adult", "minor")');
			expect(result).toContain('?');
			expect(result).toContain(':');
	...
	describe('edge cases', () => {
		...
		test('handles complex nested expressions', () => {
			const result = converter.convert(
				'if(${age} > 18, concat(${first}, " ", ${last}), "minor")'
			);
			expect(result).toContain('?');
			expect(result).toContain(':');
			expect(result).toContain('+');
	...
	describe('real-world examples', () => {
		test('converts ODK relevance expression', () => {
			const result = converter.convert(
